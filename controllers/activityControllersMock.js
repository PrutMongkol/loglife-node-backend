...
export const listActivities = async (req, res) => {
  const userId = new ObjectId(mockUserId);
...
export const createActivity = async (req, res) => {
  if (!ObjectId.isValid(mockUserId)) {
  ...
  activity.userId = new ObjectId(mockUserId);
  ...
  try {
    ...
    updateMonthlySummary( activity.date , mockUserId );
  } catch {
    res.status(500).send(error.message);
  }
};
